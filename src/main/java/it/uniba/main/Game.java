	...
	void move(String move) throws IllegalMoveException {
		...
		if (move.length()==2){
			...
			if ((board[x-1][y].getPiece() instanceof Pawn )&& (board[x-1][y].getPiece().getColor() == 1) && whiteTurn==false) { //neri
				...
				else throw new IllegalMoveException("Illegal move. Please try again.");
			}
			...
			if (z == y-1) {
				if (board[x][y] != null) {
					if (board[x-1][y-1].getPiece() instanceof Pawn) {       //cattura in diagonale da sinistra
						...
						if ((board[x][y].getPiece() instanceof Pawn) && (board[x][y].getPiece().getColor() != p.getColor())) {
				...
				if (board[x][y] != null) {
					if (board[x-1][y+1].getPiece() instanceof Pawn) { //cattura in diagonale da destra
						...
						if ((board[x][y].getPiece() instanceof Pawn) && (board[x][y].getPiece().getColor() != p.getColor())) {
							...
							board[x][y].setPiece(p);
							board[x-1][y+1].setEmpty();
							movesDone.add(p.getType() +" captured "+ caught.getType() + " on " + move.substring(2,4));  //da implementare stampa
							this.WhitesCaptured.add(caught);
							whiteTurn=true;
							System.out.println(p.getType() +" captured "+ caught.getType() + " on " + move.substring(2,4));
						} 
					...
					if (board[x-1][y-1].getPiece() instanceof Pawn) {       //cattura en Passant in diagonale da sinistra
						if (board[x-1][y].getPiece() instanceof Pawn) {
							...
							if ((board[x][y].getPiece() == null)&& (caught.enPassantCatturable(x-1))) {
								...
								board[x-1][y-1].setEmpty();
								board[x-1][y].setEmpty();
								movesDone.add(p.getType() +" captured "+ caught.getType() + " on " + move.substring(2,4) + " e.p.");
								this.WhitesCaptured.add(caught);
								whiteTurn=true;
