...
class King extends Piece {
   ...
   * Inoltre, alla creazione dell'oggetto, inizializza le coordinate del Re.
   *
  ...
  /**
   * Chiama il metodo isThreatened sulla posizione del Re utilizzato dal giocatore che sta giocando
   * in quel turno.
   *
   * @return true, se il Re del giocatore che sta giocando in quel turno e' sotto scacco; false,
   *     altrimenti.
   */
   ...
   * @return true, se il Re del giocatore che sta giocando in quel turno e' minacciato; false
   *     altrimenti.
   */
  private static boolean checkPawnThreat(final int x, final int y) {
    ...
    if (blackTurn) {
      if (x < OUTOFBOUND) {
        ...
        if (y > 0 && checkPiece instanceof Pawn && checkPiece.getColor() == 1) {
          return true;
        }
        checkPiece = Game.getCell(x + 1, y + 1).getPiece();
        if (y < OUTOFBOUND && checkPiece instanceof Pawn && checkPiece.getColor() == 1) {
          return true;
        }
      }
    }
    if (!blackTurn) {
      if (x > 0) {
        checkPiece = Game.getCell(x - 1, y - 1).getPiece();
        if (y > 0 && checkPiece instanceof Pawn && checkPiece.getColor() == 0) {
          return true;
        }
        checkPiece = Game.getCell(x - 1, y + 1).getPiece();
        if (y < OUTOFBOUND && checkPiece instanceof Pawn && checkPiece.getColor() == 0) {
          return true;
        }
      }
    }
  ...
  private static boolean checkBishopThreat(final int x, final int y) {
    ...
    while (x - i >= 0 && y + j <= OUTOFBOUND) {
      ...
      if (checkPiece != null) {
        if (checkPiece.getColor() == blackTurnColor || checkPiece instanceof Pawn) {
          break;
        }
  ...
  /**
   * Controlla che il Re non sia minacciato da una Torre e dalla Regina orizzontalmente e
   * verticalmente.
   *
   * @param x ascissa da controllare.
   * @param y ordinata da controllare.
  ...
  /**
   * Permette di muovere il Re all'interno della scacchiera e comprende anche la possibilita' di
  ...
  static String[] move(final String move) throws IllegalMoveException {
    ...
    // cella di destinazione da ritornare, parti da colonna specificata e tagli fino alla fine della
    // stringa
  ...
  static String[] castling(final boolean isLong) throws IllegalMoveException {
    ...
    if (!(Game.getCell(xK, yK).getPiece() instanceof King)
        || !(Game.getCell(xK, yR).getPiece() instanceof Rook)) {
      throw new IllegalMoveException(
          "Mossa illegale; Impossibile effettuare arrocco lungo, "
              + "re o torre non sono nella posizione iniziale");
    }
    ...
    if ((k.getnMoves() != 0) || (r.getnMoves() != 0)) {
      throw new IllegalMoveException(
