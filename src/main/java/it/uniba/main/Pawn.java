...
class Pawn extends Piece {
  ...
  static String[] move(final String move) throws IllegalMoveException {
    ...
    Pawn p;
    String[] pieceAndCell =
        new String[STRARRDIM]; // 0 pezzo che viene mosso, //2 cella di destinazione
    ...
    if (King.isThreatened()) {
      ...
      throw new IllegalMoveException(
          "Mossa illegale; il Re è sotto scacco o ci finirebbe dopo questa mossa");
    }
    p.incrementMoves();
  ...
  /**
   * Consente ad un Pedone di catturare, semplicemente o en passant, un pezzo all'interno della
   * scacchiera.
   *
  ...
  static String[] capture(final String move) throws IllegalMoveException {
    ...
    Piece p, caught;
    String[] pieces =
        new String[STRARRDIM]; // 0 pezzo che cattura, 1 pezzo catturato, 2 cella di destinazione
  ...
  static String[] captureEnPassant(final String move) throws IllegalMoveException {
    ...
    Piece p;
    String[] pieces =
        new String[STRARRDIM]; // 0 pezzo catturato, 1 pezzo che cattura //2 cella di destinazione
    y = (int) (move.charAt(CHARPOS2)) - AINASCII;
    x = MAXROW - (((int) move.charAt(CHARPOS3)) - DIGIT0INASCII);
    if ((x < 0) || (x > OUTOFBOUND) || (y < 0) || (y > OUTOFBOUND)) {
      throw new IllegalMoveException("Mossa illegale; non rispetta i limiti della scacchiera");
