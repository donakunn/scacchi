 ...
 * <p><I>Titolo</I>: Knight
 *
 * <p><I>Descrizione</I>: La classe Knight implementa la classe astratta {@link Piece} e permette di
...
class Knight extends Piece {
  ...
  static String[] move(final String move) throws IllegalMoveException {
    ...
    if (count <= 2) {
      if ((move.length() > PIECEMOVELENGTH)
          && (move.charAt(CHARPOS1) >= '1' && move.charAt(CHARPOS1) <= '8')) {
        if (xC1 != (MAXROW - Integer.parseInt(move.substring(CHARPOS1, CHARPOS2)))) {
          throw new IllegalMoveException(
              "Mossa illegale; Nessun cavallo puo' muoversi a partire dalla riga data");
        }
      } else if ((move.length() > PIECEMOVELENGTH)
          && (move.charAt(CHARPOS1) >= 'a' && move.charAt(CHARPOS1) <= 'h')) {
        if (yC1 != ((int) move.charAt(CHARPOS1) - AINASCII)) {
          throw new IllegalMoveException(
              "Mossa illegale; Nessun cavallo puo' muoversi a partire dalla colonna data");
        }
      }
    }
