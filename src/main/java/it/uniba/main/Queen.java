 ...
 * caratteristiche e comportamento della Regina all'interno del gioco
 *
...
class Queen extends Piece {
  ...
  static String[] move(final String move) throws IllegalMoveException {
    ...
    if (move.length() == PIECEMOVELENGTH) {
      isCapture = false;
    } else if ((move.length() == CAPTURELENGTH)
        && (move.substring(CHARPOS1, CHARPOS2).equals("x"))) {
      isCapture = true;
      x = xValue;
      y = 2;
    } else {
   ...
   * @return array contenente la Regina che e' stata mossa convertito a stringa, l'eventuale pezzo
   * catturato convertito a stringa (null in caso di mossa semplice) e la cella di destinazione.
   * @throws IllegalMoveException eccezione per mossa illegale.
   */
